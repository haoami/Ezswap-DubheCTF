...
contract Pool is IPool, PoolTicksState, ERC20('KyberSwap v2 Reinvestment Token', 'KS2-RT') {
      ...
      if (swapData.sqrtP != swapData.nextSqrtP) {
        if (swapData.sqrtP != swapData.startSqrtP) {
          if (swapData.currentTick < swapData.nextTick) {
            require(swapData.sqrtP < swapData.nextSqrtP);
          }
  ...
  function flash(
    ...
    if (paid0 > 0) token0.safeTransfer(msg.sender, paid0);
    if (paid1 > 0) token1.safeTransfer(msg.sender, paid1);
